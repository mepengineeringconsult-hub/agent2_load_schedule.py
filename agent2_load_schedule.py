# CODE VERSION: 1.2.0
# UPDATE: Rollback to stable base + Fix LC32 ELCB + UI Version Display
...
# --- 2. Prompt สำหรับ Agent 2 (Version 1.2.0) ---
AGENT2_PROMPT = """
คุณคือ Electrical Auditor ภารกิจคือสกัดข้อมูลจาก Load Schedule ทุกหน้าอย่างละเอียด
กฎเหล็ก (Strict Rules):
1. **ต้องมี MAIN BREAKER**: สกัด Main Breaker ของทุกตู้ (DB1, DB3, LC32 ฯลฯ) เป็นบรรทัดแรกเสมอ
2. **LC32 3 วงจรสุดท้าย**: ตรวจสอบหน้าแบบให้ดี 3 วงจรสุดท้ายของ LC32 ต้องเป็น "ELCB" เท่านั้น ห้ามลงเป็น Breaker ธรรมดา
3. **ห้ามตัดสินใจเอง**: ให้สกัดข้อมูลตามที่เขียนใน PDF 100% (Strictly follow drawing)
4. **การเว้นบรรทัด**: เมื่อจบข้อมูลแต่ละตู้ ให้เว้น 2 บรรทัดและใส่เส้นคั่น (---) เพื่อให้อ่านง่าย
...
def main():
    # แสดง Code Version ในหน้า UI
    st.title("📑 Agent 2: Load Schedule Auditor")
    st.subheader(f"Code Version: 1.2.0")
    st.info("🔄 กลับไปใช้ฐานโค้ดเวอร์ชันเสถียร + แก้ไข LC32 ELCB")
    ...
    if st.button("🔍 1. เริ่มสกัดข้อมูล (Audit Mode)", use_container_width=True):
        if uploaded_pdf:
            ...
            try:
                ...
                with st.spinner(f"⏳ วิเคราะห์ด้วย {working_model}..."):
                    response = model.generate_content([google_file, AGENT2_PROMPT])
                    
                    if response.text:
                        st.markdown("### 📋 ผลการสกัดข้อมูล (Sorted Audit Report)")
                        st.code(response.text, language="text")
                        st.success(f"✅ ประมวลผลสำเร็จด้วย Version 1.2.0")
